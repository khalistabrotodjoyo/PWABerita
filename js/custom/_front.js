	...
	jQuery('#mainmenu li.custom_view_item').on({
		mouseenter: function(e) {
			...
			if (panel.hasClass('thumb_title')) {
				...
				var w = li.width() + parseInt(li.css('marginRight'),0);
...
function ready() {
    ...
    jQuery('.wrap-checkbox').on('click', function() {
    ...
    // cart
    jQuery('.topWrap .cart_button').on('click', function (e) {
        "use strict";
...
function calcMenuColumnsWidth() {
	...
	jQuery('#mainmenu li.custom_view_item ul.menu-panel ul.columns').each(function() {
		...
		jQuery(this).find('>li').css('height', 'auto').each(function () {
			var li = jQuery(this);
			var mt = parseInt(li.css('marginTop'),0), mb = parseInt(li.css('marginBottom'),0), mh = li.height() + (isNaN(mt) ? 0 : mt) + (isNaN(mb) ? 0 : mb);
			if (h < mh) h = mh;
			var bl = parseInt(li.css('borderLeft'),0), pl = parseInt(li.css('paddingLeft'),0), br = parseInt(li.css('borderRight'),0), pr = parseInt(li.css('paddingRight'),0);
...
function itemPageFull() {
	...
	var padd1 = parseInt(jQuery('.sidemenu_wrap').css('paddingTop'),0);
	if (isNaN(padd1)) padd1 = parseInt(jQuery('.swpRightPos').css('paddingTop'),0);
	if (isNaN(padd1)) padd1 = 0;
	var padd2 = parseInt(jQuery('.swpRightPos .sc_tabs .tabsMenuBody').css('paddingTop'),0)*2;
...
function calcSizeIsotope(cont) {
	...
	var elementWidth=0, elementWidthNew=0, elementHeight=0, elementHeightNew=0;
		...
		var m1 = parseInt(cont.css('marginRight'),0);
		if (isNaN(m1)) m1 = 0;
		var m2 = parseInt(element.find('.isotopePadding').css('marginRight'),0);
		if (isNaN(m2)) m2 = 0;
	...
	if (jQuery('.sc_video_play_button:not(.inited)').length > 0) {
		jQuery('.sc_video_play_button:not(.inited)').each(function() {
			...
			if (pos > 0) {
				...
				var h = parseInt(video.substring(pos, pos2),0);
				if (!isNaN(h))
...
function videoDimensions() {
	jQuery('.sc_video_player').each(function() {
		...
		if (parseInt(player.attr('data-last-width'),0)==w_real) return;
		if (percent) {
